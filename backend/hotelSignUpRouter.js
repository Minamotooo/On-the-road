...
hotelSignUpRouter.post('/divisions', async (req, res) => {
    ...
    try {
       ...
       console.log("Request received");
            const result = await pool.query('SELECT name FROM divisions ORDER BY name;');
  ...
  hotelSignUpRouter.post('/districts', async (req, res) => {
    ...
    try {
       ...
       console.log("Request received");
            const result = await pool.query('SELECT name FROM districts WHERE division_id = ( SELECT division_id FROM divisions WHERE name = $1) ORDER BY name;',[division]);
            res.json({success : true, data:result.rows}); 
            console.log(result.rows);
  ...
  hotelSignUpRouter.post('/upazillas', async (req, res) => {
    ...
    try {
       ...
       console.log("Request received");
            const result = await pool.query('SELECT name FROM upazillas WHERE district_id = ( SELECT district_id FROM districts WHERE name = $1) ORDER BY name;',[district]);
            res.json({success : true, data:result.rows}); 
            console.log(result.rows);
  ...
  hotelSignUpRouter.post('/unions', async (req, res) => {
    ...
    try {
       ...
       console.log("Request received");
            const result = await pool.query('SELECT name FROM unions WHERE upazilla_id = ( SELECT upazilla_id FROM upazillas WHERE name = $1) ORDER BY name;',[upazilla]);
            res.json({success : true, data:result.rows}); 
            console.log(result.rows);
  ...
  hotelSignUpRouter.post('/', async (req, res) => {
   ...
   // console.log(hashedPassword);
    try {
      ...
      const result1 = await pool.query('INSERT INTO BUSINESS_ENTITY (USERNAME, BUSINESS_TYPE) VALUES ($1, $2 ) RETURNING *', [username, businessType]);
      //console.log(result1);
      ...
      const result2 = await pool.query('INSERT INTO HOTEL (USERNAME, PASSWORD, NAME, UNION_ID, ADDRESS, PHONE_NO, EMAIL) VALUES ($1, $2, $3, $4, $5, $6, $7 ) RETURNING *', [username, hashedPassword, hotelname,  union_id,  hoteladdress, hotelphonenumber, email]);
      //console.log(result2);
